...
object LineOfSight {
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    /** val m = if (from == 0) max(0, startingAngle) else max(input(from)/from, startingAngle)
    output(from) = m
    downsweepSequential(input, output, m, from + 1, until) */
    var i = from
    var m: Float = startingAngle
    while (i<until) {
      output(i) = max(m, input(i)/i)
      m = output(i)
      i = i + 1
    }
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    tree: Tree): Unit = {
    tree match {
      case node: Node => parallel(downsweep(input, output, startingAngle, node.left),
                        downsweep(input, output, max(startingAngle, node.maxPrevious), node.right))
